...
module.registerHandlers({
  google: {
    handleCallbackUrl: async ({
      context,
      input,
      makeError,
    }) => {
      const sessionCk = makeOauthSessionCacheKey(
        input.sessionId
      );

      const cachedSession =
        await cache.get<OauthSession>(sessionCk);
      ...
      const authResponse =
        await google.requestAuthenticationToken({
          code: input.code,
      ...
      const { data: profile } = await callAction(
        AuthAction.google.getUserProfile,
        {
          input: {
